...
class ExtractFeatures(object):
    def __init__(self, config):
        ...
        self.scaleV = []
        self.selected = []
        self.narcoFeatureSelectPath = config.narco_feature_selection_path
        self.scalingPath = config.narco_scaling_path
        self.resolution = config.resolution
        self.moving_avg_resolution_seconds = 150
        # self.moving_avg_epochs = self.moving_avg_resolution_seconds // self.resolution
        self.moving_avg_epochs = self.moving_avg_resolution_seconds // 15

    def extract(self, hyp, hyp_30s=None):
        eps = 1e-10
        ...
        if hyp_30s is not None:
            ...
            rem = hyp.shape[0] % int(30 // self.resolution)
            if rem > 0:
                data = hyp[:-rem, :]
            else:
                data = hyp

            data = data.reshape([-1, int(30 // self.resolution), 5])
    ...
    def select_features(self, threshold=1):
        ...
        if len(self.selected) == 0:
            with open(os.path.join(self.narcoFeatureSelectPath, "narcoFeatureSelect.p"), "rb") as sel:
                S = pickle.load(sel)
    ...
    def scale_features(self, features, sc_mod):
        scaled_features = features
        if len(self.meanV) == 0:
            with open(os.path.join(self.scalingPath, sc_mod + "_scale.p"), "rb") as sca:
