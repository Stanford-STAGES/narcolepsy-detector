...
def plot_roc_ensemble(y_true, y_pred, thr_val=None, title=None, figtitle=None, savepath=None):
    ...
    ax = plt.gca()
    ax_insert = ax.inset_axes([0.12, 0.42, 0.26, 0.26], transform=ax.transData)

    # for intelligent zoom:
    x_vals = []
    y_vals = []

    for y_t, y_p, t in zip(y_true[:-1], y_pred[:-1], title[:-1]):
        fpr, tpr, thr = roc_curve(y_t, y_p)
        ax_insert.plot(fpr, tpr, ":", lw=lw)
        if not isinstance(thr, list):
            thr_idx = np.argmax(thr < thr_val)
            ax_insert.plot(fpr[thr_idx], tpr[thr_idx], color="navy", marker="o", markersize=4)
            x_vals.append(fpr[thr_idx])
            y_vals.append(tpr[thr_idx])
    # Run over the last exp, which is assumed to be the ensemble
    fpr, tpr, thr = roc_curve(y_true[-1], y_pred[-1])
    roc_auc = auc(fpr, tpr)
    ax_insert.plot(fpr, tpr, lw=lw)
    ...
    x1, x2, y1, y2 = max(0.0, min(x_vals) - 0.01), max(x_vals) + 0.01, min(y_vals) - 0.01, min(1.0, max(y_vals) + 0.01)
    ax_insert.set_xlim(x1, x2)
    ax_insert.set_ylim(y1, y2)
